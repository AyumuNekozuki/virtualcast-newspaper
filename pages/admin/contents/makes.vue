        ...
        <div class="editor_body">
          <div class="input_area">
            ...
            <div id="input_imgs_form">
              <div id="input_imgs">
                <div class='input_text'>
                  <label>1ページ目</label><input type='file' accept='img/*' name='input_1' id='input_file_1' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
                <div class='input_text'>
                  <label>2ページ目</label><input type='file' accept='img/*' name='input_2' id='input_file_2' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
                <div class='input_text'>
                  <label>3ページ目</label><input type='file' accept='img/*' name='input_3' id='input_file_3' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
                <div class='input_text'>
                  <label>4ページ目</label><input type='file' accept='img/*' name='input_4' id='input_file_4' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
                <div class='input_text'>
                  <label>5ページ目</label><input type='file' accept='img/*' name='input_5' id='input_file_5' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
                <div class='input_text'>
                  <label>6ページ目</label><input type='file' accept='img/*' name='input_6' id='input_file_6' class="input_files" @change='changeImg'>
                  <button class="file_input_delete"><font-awesome-icon :icon="['fas', 'trash-alt']"/></button>
                </div>
              </div>
            </div>
  ...
  mounted(){
    ...
    if(process.client){
      ...
      $('.file_input_delete').on('click', function(){
        var index = $('.file_input_delete').index(this);
        ($('.input_files').eq(index)).val(null);
        ($('.watch_area img').eq(index)).remove();
      })
  ...
  methods: {
    changeImg(e){
      ...
      if (this.thumbnail) {
        ...
        reader.onload = () => {
          $('#WatchApp .watch_area').append("<img src='"+ reader.result +"'>");
        }
      }
    },
    append_data(){
      ...
      if(check){
        ...
        for(var i = 0; i < 6; i++){
          var is_check_imgs = ($('.input_files').eq(i)).val();
          if(is_check_imgs){
            var path = storageRef.child('/archives/'+ archive_id + '/' + (i + 1) +'.png');
            var thumbnail = ($('.input_files').eq(i)).prop('files')[0];
            path.put(thumbnail)
              .then(console.log("VCNP: 画像"+ (i+1) +"枚目のアップロードが完了しました。"))
              .catch(error => console.error(error))

            if(i == 0){
              var path = storageRef.child('/archives/'+ archive_id + '/head.png');
              var thumbnail = ($('.input_files').eq(i)).prop('files')[0];
              path.put(thumbnail)
